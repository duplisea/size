...
The PLF for all species with a 30 cm threshold between small and large individuals
```{r PLFgf}
PLF.gf= PLF.f(species.group="pelagic",cutoff=30)
```

The PLF for all species with a 30 cm threshold between small and large individuals
```{r PLFredish}
PLF.rf= PLF.f(species.group="code",code.qc=792,cutoff=20)
```
...
plot(PLF.all$year, PLF.all$plf,xlab="Year",ylab="Proportion of large individuals",type="l",lwd=2,
  ylim=c(0,ymax))
lines(PLF.dem$year,PLF.dem$plf,col="blue")
```
